...
/* ***************************************************** */
/* NEW FUNCTIONS FOR ATTACKS USING MAGMA NORM FUNCTION   */
/*        MIGHT WORK ONLY FOR KF OF DEGREE p^2           */
/* ***************************************************** */ 


/**
 * List of indices to re-order elements in a change of representation.
 * kf is of the form <[m1, m2, ..., mr], p> and kf_sub is <[n1, ..., n{r-1}], p>.
 */
function KF_coord_perm(kf_new, kf)
    B1, _ := KF_primefree_basis(kf[1], kf[2]);
    B, _ := KF_primefree_basis(kf_new[1], kf_new[2]);
    l := [];
    for _b in B do
        Append(~l, Index(B1, _b));
    end for;
    return l;
end function;

/**
 * Uses KF_coord_perm to permute coordinates of element `g`.
 */
function KF_coord_perm_elt(g, kf_new, kf : l:=[])
    if #l eq 0 then
	l := KF_coord_perm(kf_new, kf);
    end if;
    gg := [];
    for _l in l do
	Append(~gg, g[_l]);
...
/**
 * Same as KF_coord_perm_elt but with a sequence of elts `L`.
 */
function KF_coord_perm_family (L, kf_new, kf : l:=[])
    if #l eq 0 then
	l := KF_coord_perm(kf_new, kf);
    end if;
    LL := [];
...
/**
 * Computes a random element of size `s` in the ideal `I` of order `O` generated by `g`.
 */
function KF_random_ideal_elt (g, O, s : I:=0*O)
    if I eq 0*O then
	I := g*O;
    end if;
    return Random(I, s);
...
/**
 * Computes a two-element representation of the ideal g*O.
 */
function KF_two_elts_order (g, O : s:= 10 )
    n := Generator(g*O meet Integers());
    I := g * O;
    c:= 0;
    print "before repeat";
    ...
    repeat
	c +:= 1;
	alpha := KF_random_ideal_elt(g, O, s: I:=I);
	alpha := Denominator(alpha) * alpha;
    until GCD(Integers()!AbsoluteNorm(alpha), Integers()!n) eq Integers()!n;
	/* J := ideal < O | [n, alpha] >; */
	/* print c; */
	/* time AbsoluteNorm(J); */
    /* until AbsoluteNorm(J)  eq AbsoluteNorm(I); */
    return [n, alpha];
end function;
...
/*
 * L/K is an extension, defined by kf and kf_sub
 * OL is an extension order of OKabs, where Kabs is the absolute field rep of K.
 * g is in L
 */
KF_vec_to_order := function(g, kf, kf_sub, O, OKabs, K, L)
    d := Degree(O);
    n := #g div d;
    h := Vector_to_NFelement1(g, kf_sub[1] cat [kf[1][#kf[1]]], kf[2]: KF:=L );
    h := ElementToSequence(h);
    print h;
    /* h := [g[1+i*n..(i+1)*n] : i in [0..d-1]]; */
    /* h := [ OKabs ! _h : _h in h ]; */
    return FieldOfFractions(O)!h;
end function;
