...
KF_multifinite_morphism := function(U, field_sequence, field_exponent) /* Calculate good morphism for U from K to F_p1 x ...x F_pr */
    Q := [];
    field_length := #field_sequence;
    field_dim := field_exponent^field_length;
    r := #U;
	...
	F_q := GF(q);
	/* "calculating basis"; */
	basis_q := KF_naive_basis_q(field_sequence, field_exponent, F_q);
	t := 0;
	E := [];
	for i in [1..r] do
	    /* "calculating embedding"; */
	    e := Embedding_q(U[i], field_sequence, field_exponent, F_q, basis_q);
	    if (e eq 0) then
		break;
	    else
		t := t + 1;
	...
	if (t eq r) then			/* test if q is good for every element in U */
		...
		Q := Q1;
		for i in [1..#E] do
		    Append(~V[i], Rationals()!(Integers()!E[i]));
		end for;
	    end if;
	end if;   
    end while;
...
/* p-adic characters of U */
KF_characters_from_field := function(U, Q, field_sequence, field_exponent)
    M := [];
    r := #U;
    for i in [1..#Q] do
	q := Q[i];
	F_q := GF(q);
	zeta := RootOfUnity(field_exponent, F_q);			/* calculate zeta_p in F_q and Log(zeta_p, CubeCharacter_q) */
	Char_q := [(F_q!U[j, i])^((q-1) div field_exponent) : j in [1..r]];
	Row_q := [ Log(zeta, (Char_q)[j]) : j in [1..r] ];
	Append(~M, Row_q);
    end for;
    return Matrix(GF(field_exponent), M);
